    ...
    >
      <div
        ...
        style={{
          ...
          background:'linear-gradient(145deg, rgba(18,14,38,0.99), rgba(8,6,20,1))',
      ...
      >
        ...
        <div style={{ padding:'26px 26px 22px' }}>
          ...
          <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'18px' }}>
            ...
            <button
              onClick={onClose}
              style={{ background:'none', border:'none', cursor:'pointer', color:'rgba(107,114,128,0.45)', padding:'4px', display:'flex', transition:'color 0.15s', marginTop:'-2px' }}
          ...
          {/* Title */}
          <h3 style={{ fontFamily:"'Cinzel',serif", fontSize:'15px', fontWeight:700, color:'white', margin:'0 0 8px', letterSpacing:'0.04em' }}>
            {modal.title || 'Notice'}
          </h3>

          {/* Message */}
          <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'13px', color:'rgba(156,163,175,0.85)', margin:'0 0 22px', fontWeight:300, lineHeight:1.65 }}>
            {modal.message}
          </p>
          ...
          <button
            ...
            onMouseEnter={e => {
              e.currentTarget.style.background = isError
                ...
                : isWarning ? 'rgba(251,191,36,0.14)' : 'rgba(124,58,237,0.16)';
            }}
    ...
    <>
      ...
      <div style={{ minHeight:'100vh', background:'#06060f', fontFamily:"'DM Sans',sans-serif", position:'relative', overflow:'hidden' }}>
        ...
        <div className="page-inner" style={{
          ...
          maxWidth:'720px', margin:'0 auto', padding:'96px 32px 48px',
          ...
          <div style={{
            ...
            border:'1px solid rgba(124,58,237,0.15)', borderRadius:'22px', overflow:'hidden',
            ...
            <form onSubmit={handleAddKeys} style={{ padding:'28px 32px', display:'flex', flexDirection:'column', gap:'22px' }}>
              ...
              <div>
                ...
                <div style={{ position:'relative' }}>
                  <select
                    value={selectedSoft}
                    onChange={e => { setSelectedSoft(e.target.value); setDuration(''); }}
                    onFocus={() => setFocused('soft')}
                    onBlur={() => setFocused('')}
                    style={{
                      width:'100%', boxSizing:'border-box',
                      padding:'12px 36px 12px 14px',
                      background:focused==='soft'?'rgba(124,58,237,0.05)':'rgba(6,6,18,0.8)',
                      border:focused==='soft'?'1px solid rgba(124,58,237,0.6)':'1px solid rgba(55,48,80,0.7)',
                      borderRadius:'12px', color:selectedSoft?'white':'rgba(107,114,128,0.5)',
                      fontSize:'14px', fontFamily:"'DM Sans',sans-serif",
                      outline:'none', cursor:'pointer', transition:'all 0.2s',
                      appearance:'none', WebkitAppearance:'none',
                      boxShadow:focused==='soft'?'0 0 0 3px rgba(124,58,237,0.1)':'none',
                    }}
                  >
              ...
              {selectedSoft && (
                <div>
                  ...
                  {durationPlans.length === 0 ? (
                    ...
                    <div className="dur-grid" style={{ display:'grid', gridTemplateColumns:`repeat(${durationPlans.length},1fr)`, gap:'8px' }}>
                      {durationPlans.map(plan => (
                        <label key={plan.value} className={`dur-card ${duration===plan.value?'active':''}`} onClick={() => setDuration(plan.value)}>
                          ...
                          <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'11px', fontWeight:600, letterSpacing:'0.02em', lineHeight:1, color:duration===plan.value?'#c4b5fd':'rgba(156,163,175,0.8)' }}>
                            {plan.label}
                          </span>
                          <span style={{ fontFamily:"'Cinzel',serif", fontSize:'11px', fontWeight:700, lineHeight:1, color:duration===plan.value?'#a78bfa':'rgba(107,114,128,0.65)' }}>
                            ${currentSoftware.prices[plan.field].toFixed(2)}
                          </span>
                        </label>
                      ))}
                    </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  style={{
                    ...
                    background:focused==='keys'?'rgba(124,58,237,0.04)':'rgba(6,6,18,0.8)',
                    border:focused==='keys'?'1px solid rgba(124,58,237,0.6)':'1px solid rgba(55,48,80,0.7)',
                    borderRadius:'12px', color:'white',
                    fontSize:'13px', fontFamily:"'DM Mono',monospace",
                    outline:'none', resize:'vertical', transition:'all 0.2s', lineHeight:1.7,
                    boxShadow:focused==='keys'?'0 0 0 3px rgba(124,58,237,0.1)':'none',
                  }}
                />
                {keysInput.trim() && (
                  <div style={{ display:'flex', alignItems:'center', gap:'6px', marginTop:'8px' }}>
                    <div style={{ width:'6px', height:'6px', borderRadius:'50%', background:'#34d399', boxShadow:'0 0 6px #34d399' }} />
                    <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'12px', color:'#34d399', fontWeight:500 }}>
                      {keyCount} {keyCount===1?'key':'keys'} detected
                    </span>
                  </div>
                )}
              </div>
              ...
              <button type="submit" disabled={isLoading} className="submit-btn"
                style={{
                  ...
                  color:'white', fontSize:'13px', fontFamily:"'Cinzel',serif", fontWeight:600, letterSpacing:'0.08em',
      ...
      <div style={{
        ...
        border:'1px solid rgba(52,211,153,0.3)', borderRadius:'12px', padding:'12px 18px',
