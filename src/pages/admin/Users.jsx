...
import { ArrowLeft, UserCheck, UserX, Trash2, Users as UsersIcon, DollarSign, X, AlertTriangle, CheckCircle } from 'lucide-react';
    ...
    <Modal show={show} onClose={onClose}>
      <div style={{ padding: '28px 28px 24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '22px' }}>
          ...
          <button onClick={onClose} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '8px', padding: '6px', color: '#6b7280', cursor: 'pointer', transition: 'all 0.2s', display: 'flex' }}>
            <X size={16} />
          </button>
        </div>
        ...
        <div style={{ background: 'rgba(52,211,153,0.06)', border: '1px solid rgba(52,211,153,0.15)', borderRadius: '10px', padding: '12px 16px', marginBottom: '18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <span style={{ fontFamily: "'Cinzel', serif", fontSize: '16px', color: '#34d399', fontWeight: 700 }}>
        ...
        <div style={{ marginBottom: '14px' }}>
          <label style={{ display: 'block', marginBottom: '8px', fontFamily: "'DM Sans', sans-serif", fontSize: '12px', fontWeight: 500, color: 'rgba(156,163,175,0.8)', letterSpacing: '0.04em' }}>
            Amount to Add ($)
          </label>
          <div style={{ position: 'relative' }}>
            ...
            <input
              ...
              style={{
                ...
                background: focused ? 'rgba(124,58,237,0.05)' : 'rgba(6,6,18,0.8)',
                border: focused ? '1px solid rgba(124,58,237,0.6)' : '1px solid rgba(55,48,80,0.7)',
                borderRadius: '12px', color: 'white',
                fontSize: '15px', fontFamily: "'Cinzel', serif", fontWeight: 600,
        ...
        <button
          ...
          style={{
            ...
            background: confirmZero ? 'rgba(248,113,113,0.12)' : 'rgba(255,255,255,0.04)',
            border: confirmZero ? '1px solid rgba(248,113,113,0.4)' : '1px solid rgba(255,255,255,0.08)',
            color: confirmZero ? '#f87171' : 'rgba(156,163,175,0.6)',
...
const ConfirmModal = ({ show, type, user, onClose, onConfirm }) => {
  const configs = {
    approve: {
      ...
      iconColor: '#34d399', iconBg: 'rgba(52,211,153,0.12)', iconBorder: 'rgba(52,211,153,0.25)',
      title: 'Approve Reseller',
      message: `Are you sure you want to approve this account? They will get full access.`,
      btnLabel: 'Approve', btnStyle: 'linear-gradient(135deg, #059669, #047857)',
      btnShadow: 'rgba(5,150,105,0.35)',
    },
    disable: {
      icon: <UserX size={22} />,
      iconColor: '#fb923c', iconBg: 'rgba(251,146,60,0.12)', iconBorder: 'rgba(251,146,60,0.25)',
      title: 'Disable Account',
      message: `Are you sure you want to disable this account? They won't be able to login.`,
      btnLabel: 'Disable', btnStyle: 'linear-gradient(135deg, #ea580c, #c2410c)',
      btnShadow: 'rgba(234,88,12,0.35)',
    },
    delete: {
      icon: <Trash2 size={22} />,
      iconColor: '#f87171', iconBg: 'rgba(248,113,113,0.12)', iconBorder: 'rgba(248,113,113,0.25)',
      title: 'Delete User',
      message: `This action is permanent and cannot be undone. The user data will be lost forever.`,
      btnLabel: 'Delete', btnStyle: 'linear-gradient(135deg, #dc2626, #b91c1c)',
    ...
    <Modal show={show} onClose={onClose}>
      <div style={{ padding: '28px 28px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: cfg.iconBg, border: `1px solid ${cfg.iconBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: cfg.iconColor }}>
              {cfg.icon}
            </div>
            <div>
          ...
          <button onClick={onClose} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '8px', padding: '6px', color: '#6b7280', cursor: 'pointer', display: 'flex' }}>
            <X size={16} />
          </button>
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <button onClick={onConfirm} style={{ flex: 1, padding: '12px', borderRadius: '11px', background: cfg.btnStyle, border: 'none', color: 'white', fontSize: '13px', fontFamily: "'Cinzel', serif", fontWeight: 600, letterSpacing: '0.04em', cursor: 'pointer', transition: 'all 0.2s', boxShadow: `0 6px 20px ${cfg.btnShadow}` }}>{cfg.btnLabel}</button>
        </div>
      </div>
    </Modal>
  );
  ...
  return (
    <>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table className="users-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {usersList.length === 0 ? (
                    <tr>
                      <td colSpan="4" style={{ padding: '48px', textAlign: 'center', fontFamily: "'DM Sans',sans-serif", color: 'rgba(107,114,128,0.5)', fontSize: '14px', fontWeight: 300 }}>No resellers found.</td>
                    </tr>
                  ) : usersList.map((user, idx) => (
                    <tr key={user.id} className="usr-row" style={{ borderBottom: '1px solid rgba(124,58,237,0.07)', animationDelay: `${idx * 0.04}s` }}>
                      <td style={{ padding: '16px 20px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                          ...
                          <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'13px', color:'rgba(255,255,255,0.85)', fontWeight:400 }}>{user.email}</span>
                        </div>
                      </td>
                      <td style={{ padding: '16px 20px' }}>
      ...
      <AddBalanceModal
        ...
        onClose={() => setBalanceModal({ show: false, user: null })}
      ...
      <ConfirmModal
        ...
        onClose={() => setConfirmModal({ show: false, type: '', user: null })}
        onConfirm={() => {
